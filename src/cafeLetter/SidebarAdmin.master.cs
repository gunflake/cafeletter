...
{
    public partial class SidebarAdmin : System.Web.UI.MasterPage
    {
        CommonModule module = new CommonModule();

        protected void Page_Load(object sender, EventArgs e)
        {
            if(Session["userRank"] != null && Session["userRank"].Equals("마스터"))
            {
                AdminSupervise.Visible = true;
            }

            if (Session["userSupervise"] != null && Session["userSupervise"].Equals("O"))
            {
                UserSupervise.Visible = true;
            }
        ...
        private void ItemListPrint()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < pl_objDas.RecordCount; i++)
                {
                    ...
                    anchor.Attributes.Add("href", "/ItemSupervise/ItemList.aspx?strItemCode=" + strItemCode);
